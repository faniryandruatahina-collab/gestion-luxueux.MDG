...
import { ChevronLeft, TrendingUp, DollarSign, Package, CheckCircle, Users } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Calculer les métriques de manière sécurisée
      const totalRevenue = orders.reduce((sum: number, order: any) => {
        const revenue = parseFloat(order.total_price) || 0
        return sum + revenue
      }, 0)
      
      const totalProfit = totalRevenue * 0.36 // Exemple: 36% de marge bénéficiaire

      // Trier les commandes par date (les plus récentes en premier)
      const sortedOrders = orders
        .sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
        .slice(0, 4) // Prendre les 4 plus récentes
      ...
      setMetrics({
        ...
        revenueChange: 12.5, // Pour l'exemple
        profitChange: 8.2
      })
